...
// ScrambleText — reveals target text by scrambling characters over a duration.
...
// SlideToSubmit — drag thumb to the end to trigger onSubmit.
function initSlideToSubmit(trackEl, onSubmit, label = 'SLIDE TO SUBMIT') {
  ...
  function move(clientX) {
    ...
    if (newPos >= w - 2) {
      ...
      thumb.innerHTML = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>';
      onSubmit();
    }
  }
  ...
  window.addEventListener('mouseup', () => { if (dragging && !completed) { dragging = false; position = 0; thumb.style.transform = 'translateX(0)'; fill.style.width = '52px'; } });
  window.addEventListener('touchmove', (e) => { if (dragging) { e.preventDefault(); move(e.touches[0].clientX); } }, { passive: false });
  window.addEventListener('touchend', () => { if (dragging && !completed) { dragging = false; position = 0; thumb.style.transform = 'translateX(0)'; fill.style.width = '52px'; } });
}
